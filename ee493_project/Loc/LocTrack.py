...
visitor3 = museum.Visitor('Visitor 3')
visitor3.change_color('color3')
...
def RefreshCanvas():
    ...
    if(visitor3.found):
        c.create_oval(visitor3.x - 4, visitor3.y - 4, visitor3.x + 4, visitor3.y + 4, fill="blue")
        c.create_text(visitor3.x-8, visitor3.y+12, fill="darkblue", text='V3')
...
def track_everybody():
    ...
    while cv2.waitKey(1) != 27: # ESC
        ...
        v3_close_to = choose_recordings(visitor3.x, visitor3.y,
                                        item1=item1,
                                        item2=item2,
                                        item3=item3,
                                        door1=door1,
                                        door2=door2)
        line_holder = 0
        if visitor1.found == True:
            line_holder = line_holder + 20
            frame = cv2.putText(img,
                              'Visitor 1 close to: '+ str(v1_close_to),
                              (10, line_holder),
                              cv2.FONT_HERSHEY_SIMPLEX, 0.5, (34, 200, 34), 2)
        if visitor2.found == True:
            line_holder = line_holder + 20
            frame = cv2.putText(img,
                              'Visitor 2 close to: ' + str(v2_close_to),
                              (10, line_holder),
                              cv2.FONT_HERSHEY_SIMPLEX, 0.5, (34, 200, 34), 2)
        if visitor3.found == True and visitor1.found == True and visitor2.found == True:
            line_holder = line_holder + 20
            frame = cv2.putText(img,
                                'Maximum occupancy has been reached! Last Visitor is being warned..',
                                (10, line_holder),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (34, 200, 34), 2)
            #f = open("files/esp3", "w")
        ...
        if v1_close_to == 4: # door1
            esp1 = "door1.m4a" if vis1Door1Allow.get() else "notAllowed.m4a"
        elif v1_close_to == 5: # door2
            esp1 = "door2.m4a" if vis1Door2Allow.get() else "notAllowed.m4a"
        else:
